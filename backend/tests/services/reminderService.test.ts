...
describe('reminderService', () => {
  ...
  describe('fireDueReminders', () => {
    ...
    it('does not fire future reminders', async () => {
      await createTestReminder(event.id, { triggerAt: new Date(Date.now() + 60 * 60 * 1000), fired: false });
      const count = await fireDueReminders();
      expect(count).toBe(0);
    });
  });
  ...
  describe('createReminder', () => {
    it('creates an event-linked reminder', async () => {
      const r = await createReminder({ eventId: event.id, triggerAt: new Date(), label: 'Test', type: 'general' });
      expect(r.eventId).toBe(event.id);
    });

    it('creates a standalone reminder with no event (SCRUM-43)', async () => {
      const r = await createReminder({ triggerAt: new Date(), label: 'Standalone' });
      expect(r.eventId).toBeNull();
      expect(r.label).toBe('Standalone');
    });
  });

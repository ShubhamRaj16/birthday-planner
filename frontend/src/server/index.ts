  ...
  try {
    ...
    try {
      ...
      const js = byEntrypointOrder
        ...
        .filter(Boolean)
        .map((file) => `/static/js/${file}`) as string[];
...
const knownPatterns = (serverRoutes as RouteConfig[])
  .filter((r) => r.path !== '*')
...
function getStatusCode(req: Request): number {
...
// SSR middleware
const ssrMiddleware: RequestHandler = async (req, res) => {
        ...
        React.createElement(
          ...
          React.createElement(
            ...
            React.createElement(
              StaticRouter,
              { location: req.url },
              React.createElement(App, { routes: serverRoutes })
            )
          )
        )
      )

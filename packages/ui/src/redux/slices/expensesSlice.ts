...
export const createExpense = createAsyncThunk(
  'expenses/create',
  async ({ eventId, data }: CreateExpenseArgs, { rejectWithValue }) => {
    try {
...
export const updateExpense = createAsyncThunk(
  'expenses/update',
  async ({ eventId, id, data }: UpdateExpenseArgs, { rejectWithValue }) => {
    try {
      const res = await apiClient.put<ApiResponse<Expense>>(
        `/events/${eventId}/expenses/${id}`,
        data
      );
      return { eventId, expense: res.data.data };
    } catch (e) {
...
export const deleteExpense = createAsyncThunk(
  'expenses/delete',
  async ({ eventId, id }: DeleteExpenseArgs, { rejectWithValue }) => {
    try {

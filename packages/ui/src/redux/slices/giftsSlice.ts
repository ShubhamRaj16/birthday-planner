...
export const fetchGifts = createAsyncThunk('gifts/fetchAll', async (eventId: number, { rejectWithValue }) => {
  try {
...
export const createGift = createAsyncThunk('gifts/create', async ({ eventId, data }: CreateGiftArgs, { rejectWithValue }) => {
  try {
...
export const updateGift = createAsyncThunk('gifts/update', async ({ eventId, id, data }: UpdateGiftArgs, { rejectWithValue }) => {
  try {
...
export const deleteGift = createAsyncThunk('gifts/delete', async ({ eventId, id }: DeleteGiftArgs, { rejectWithValue }) => {
  try {
...
const giftsSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(fetchGifts.rejected, (state, action) => { state.error = action.payload as string; })
      ...
      .addCase(createGift.rejected, (state, action) => { state.error = action.payload as string; })
      ...
      .addCase(updateGift.rejected, (state, action) => { state.error = action.payload as string; })
      ...
      .addCase(deleteGift.rejected, (state, action) => { state.error = action.payload as string; });
  },
});

...
export const fetchGuests = createAsyncThunk(
  'guests/fetchAll',
  async (eventId: EventIdArg, { rejectWithValue }) => {
    try {
...
export const createGuest = createAsyncThunk(
  'guests/create',
  async ({ eventId, data }: CreateGuestArgs, { rejectWithValue }) => {
    try {
...
export const updateGuest = createAsyncThunk(
  'guests/update',
  async ({ eventId, id, data }: MutateGuestArgs, { rejectWithValue }) => {
    try {
...
export const deleteGuest = createAsyncThunk(
  'guests/delete',
  async ({ eventId, id }: DeleteGuestArgs, { rejectWithValue }) => {
    try {
...
export const bulkImportGuests = createAsyncThunk(
  'guests/bulkImport',
  async ({ eventId, guests }: BulkImportGuestsArgs, { rejectWithValue }) => {
    try {
      const res = await apiClient.post<ApiResponse<{ count: number }>>(
        `/events/${eventId}/guests/bulk-import`,
        { guests }
      );
    ...
    builder
      ...
      .addCase(deleteGuest.fulfilled, (state, action) => {
        Object.keys(state.byEventId).forEach((eid) => {
          state.byEventId[Number(eid)] = (state.byEventId[Number(eid)] || []).filter(
      ...
      .addCase(bulkImportGuests.rejected, (state, action) => {
        state.error = action.payload as string;
      });
  },
});

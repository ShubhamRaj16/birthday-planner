...
describe('BudgetTracker', () => {
  describe('summary values', () => {
    it('renders Budget label in summary bar', () => {
      renderWithStore(<BudgetTracker eventId={42} eventBudget={10000} />, makeState([]));
    ...
    it('shows total spent correctly', () => {
      ...
      renderWithStore(<BudgetTracker eventId={42} eventBudget={10000} />, makeState(expenses));
  ...
  describe('budget alerts', () => {
    it('shows warning alert at 80% spend', () => {
      const expenses = [{ id: 1, label: 'Hall', amount: 8000, paid: true, category: 'venue' }];
      renderWithStore(<BudgetTracker eventId={42} eventBudget={10000} />, makeState(expenses));
      expect(screen.getByText(/80%|Warning|warn/i)).toBeInTheDocument();
    });

    it('shows exceeded alert at 100% spend', () => {
      const expenses = [{ id: 1, label: 'Hall', amount: 10000, paid: true, category: 'venue' }];
      renderWithStore(<BudgetTracker eventId={42} eventBudget={10000} />, makeState(expenses));
      expect(screen.getByText(/exceeded|100%/i)).toBeInTheDocument();
    });

    it('shows no alert when under 80%', () => {
      const expenses = [{ id: 1, label: 'Cake', amount: 5000, paid: true, category: 'cake' }];
      renderWithStore(<BudgetTracker eventId={42} eventBudget={10000} />, makeState(expenses));
